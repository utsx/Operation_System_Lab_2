...
static ssize_t lab_driver_read(struct file *file, char __user

*buf,
size_t count, loff_t
*ppos);

static ssize_t lab_driver_write(struct file *file, const char __user
...
static int write_pci_dev(struct task_struct *task, char __user
...
static int write_inode(struct task_struct *task, char __user
...
if(task == NULL){
printk(KERN_INFO
"lab_driver: read() - task is NULL\r");
return -
EFAULT;
}
if(struct_id == STRUCT_PCI_DEV){
printk(KERN_INFO
"lab_driver: read() - struct_id == STRUCT_PCI_DEV\r");
return
write_pci_dev(task, buf, count, ppos
);
}
if(struct_id == STRUCT_INODE){
printk(KERN_INFO
"lab_driver: read() - struct_id == STRUCT_INODE\r");
return
write_inode(task, buf, count, ppos
);
}
}
return -
...
if(
copy_from_user(procfs_buffer, buf, procfs_buffer_size
)) {
return -
EFAULT;
}
...
if(num_of_args != 2){
printk(KERN_INFO
"lab_driver: write() - wrong number of arguments\r");
return -
EFAULT;
}
struct_id = a;
...
len +=
sprintf(procfs_buffer
+ len, "subsystem_vendor: %d\n", user_dev.subsystem_vendor);
len +=
sprintf(procfs_buffer
+ len, "subsystem_device: %d\n", user_dev.subsystem_device);
if (*ppos >= len ||
copy_to_user(buf, procfs_buffer, len
)){
printk(KERN_INFO
"lab_driver: read() - copy_to_user() failed\r");
return -
EFAULT;
}
*ppos +=
...
//get inode struct and return it as user_inode
...
printk(KERN_INFO
"lab_driver: read() - i_bytes: %d\r", user_inode.i_bytes);
size_t len = 0;
len +=
sprintf(procfs_buffer
+ len, "i_bytes: %d\n", user_inode.i_bytes);
...
lab_driver_init(void) {
    ...
    if (register_chrdev(0, "lab_driver", &fops) < 0) {
        printk(KERN_INFO
        "lab_driver: cannot register the device\r");
    }

    proc_create("info", 0666, proc_root, &fops);
    printk(KERN_INFO
    "lab_driver: registered\r");
    return SUCCESS;
}
